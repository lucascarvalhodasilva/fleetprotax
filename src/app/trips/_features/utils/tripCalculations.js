 ...
 * @returns {Object} { duration: number, mealAllowance: number }
 */
export const calculateMealAllowance = (startDateStr, startTimeStr, endDateStr, endTimeStr, taxRates) => {
  ...
  if (isSameDay) {
    // One day trip: > 8h = 14€
    if (diff > 8) {
      mealAllowance = taxRates.mealRate8h;
    }
  } else {
    // Multi-day trip
    ...
    mealAllowance += intermediateDays * taxRates.mealRate24h;
  }

  return { duration: diff, mealAllowance };
};
